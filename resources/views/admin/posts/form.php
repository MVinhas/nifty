<form id="post_create" class="needs-validation" action="/posts/submit/<?= $data->post->id ?? null?>" method="POST"
    ...
    <div class="form-group">
        ...
        <select name="category_id" class="form-control" id="category_id" required>
            ...
            <option value="<?= $category->id ?>" <?php if ($data->post->category_id ?? 0 === $category->id) :?>
    ...
    <div class="form-group">
        ...
        <textarea name="excerpt" class="form-control required" rows="2" id="excerpt" required><?= $data->post->excerpt ?? '' ?>
          </textarea>
    </div>
    <div class="form-group">
        <label for="content">Content
        </label>
        <textarea name="content" class="form-control required" rows="5" id="content" required><?= $data->post->content ?? '' ?>
          </textarea>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <input type="radio" name="featured" value="0" <?php if (!isset($data->post->featured) || $data->post->featured === 0) :?> checked
            <?php endif; ?> />
        <label>Regular
        </label>
        <br>
        <input type="radio" name="featured" value="1" <?php if (isset($data->post->featured) && $data->post->featured ?? 0 === 1) :?> checked
            <?php endif; ?> />
        <label>Featured
        </label>
    </div>
